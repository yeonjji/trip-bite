...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: locale === "en" ? "Glamping" : "글램핑",
    description:
      locale === "en"
        ? "Find glamping sites across Korea."
        : "전국의 글램핑 캠핑장을 탐색하세요.",
    alternates: buildAlternates("/camping/glamping"),
  }
}
...
export default async function GlampingPage({ params, searchParams }: PageProps) {
  const { locale } = await params
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-foreground">글램핑</h1>

      <Suspense>
        <CampingFilters locale={locale} basePath="/camping/glamping" hideInduty />
      </Suspense>
