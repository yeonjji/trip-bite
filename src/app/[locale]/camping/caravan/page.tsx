...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: locale === "en" ? "Caravan" : "카라반",
    description:
      locale === "en"
        ? "Find caravan camping sites across Korea."
        : "전국의 카라반 캠핑장을 탐색하세요.",
    alternates: buildAlternates("/camping/caravan"),
  }
}
...
export default async function CaravanPage({ params, searchParams }: PageProps) {
  const { locale } = await params
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-foreground">카라반</h1>

      <Suspense>
        <CampingFilters locale={locale} basePath="/camping/caravan" hideInduty />
      </Suspense>
