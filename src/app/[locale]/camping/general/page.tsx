...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: locale === "en" ? "General Camping" : "일반야영지",
    description:
      locale === "en"
        ? "Find general camping sites across Korea."
        : "전국의 일반야영지를 탐색하세요.",
    alternates: buildAlternates("/camping/general"),
  }
}
...
export default async function GeneralCampingPage({ params, searchParams }: PageProps) {
  const { locale } = await params
  const { doNm, animalCmgCl, page: pageStr } = await searchParams
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-foreground">일반야영지</h1>

      <Suspense>
        <CampingFilters locale={locale} basePath="/camping/general" hideInduty />
      </Suspense>
