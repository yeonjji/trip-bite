...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  return {
    title: locale === "en" ? "Car Camping" : "자동차야영지",
    description:
      locale === "en"
        ? "Find car camping sites across Korea."
        : "전국의 자동차야영지를 탐색하세요.",
    alternates: buildAlternates("/camping/car"),
  }
}
...
export default async function CarCampingPage({ params, searchParams }: PageProps) {
  const { locale } = await params
  ...
  return (
    <main className="mx-auto max-w-7xl px-4 py-8">
      <h1 className="mb-6 text-2xl font-bold text-foreground">자동차야영지</h1>

      <Suspense>
        <CampingFilters locale={locale} basePath="/camping/car" hideInduty />
      </Suspense>
