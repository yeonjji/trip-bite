...
import { MapPin, ChevronDown, ParkingCircle, BadgeDollarSign, CircleDollarSign, X } from "lucide-react";
...
const FEE_OPTIONS = [
  { value: "", labelKo: "전체", labelEn: "All", icon: ParkingCircle },
  { value: "무료", labelKo: "무료", labelEn: "Free", icon: BadgeDollarSign },
  { value: "유료", labelKo: "유료", labelEn: "Paid", icon: CircleDollarSign },
];
...
export default function ParkingFilters({ locale }: ParkingFiltersProps) {
  ...
  const hasFilters = zcode || sigungu || smprcSe;

  return (
    <div className="flex flex-col gap-1">
      <div className="mb-4">
        <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-0.5">Navigation</p>
        <p className="text-xs font-bold text-slate-700 tracking-wide uppercase">Parking</p>
      </div>

      {FEE_OPTIONS.map((opt) => {
        const Icon = opt.icon;
        const isActive = smprcSe === opt.value;
        return (
          <button
            key={opt.value}
            ...
            className={cn(
              ...
              isActive ? "bg-white text-orange-700 font-bold shadow-sm" : "text-slate-500 hover:bg-orange-50 hover:text-orange-700"
            )}
          >
      ...
      {hasFilters && (
        <>
          <div className="border-t border-gray-100 my-2" />
          <button
            onClick={() => router.push(`/${locale}/facilities/parking`)}
