...
import { PlugZap, Zap, BatteryCharging, MapPin, ChevronDown, X } from "lucide-react";
...
export default function EvChargingFilters({ locale }: EvChargingFiltersProps) {
  ...
  const hasFilters = zcode || sigungu || kind;

  return (
    <div className="flex flex-col gap-1">
      <div className="mb-4">
        <p className="text-[10px] font-medium text-slate-400 uppercase tracking-widest mb-0.5">Navigation</p>
        <p className="text-xs font-bold text-slate-700 tracking-wide uppercase">EV Network</p>
      </div>
      ...
      {hasFilters && (
        <>
          <div className="border-t border-gray-100 my-2" />
          <button
            onClick={() => router.push(`/${locale}/facilities/ev-charging`)}
