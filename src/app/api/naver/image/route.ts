...
export async function GET(req: NextRequest) {
  ...
  const display = Math.min(Number(req.nextUrl.searchParams.get("display") ?? "9"), 9)
  const url = `https://openapi.naver.com/v1/search/image?query=${encodeURIComponent(query)}&display=${display}&sort=sim&filter=medium`
